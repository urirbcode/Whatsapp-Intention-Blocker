// Content script
let isIntentionPromptVisible = false;

chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'showIntentionPrompt') {
    if (!isIntentionPromptVisible) {
      showIntentionPrompt();
      isIntentionPromptVisible = true;
    }
  } else if (request.action === 'allowUsage') {
    if (isIntentionPromptVisible) {
      removeIntentionPrompt();
      isIntentionPromptVisible = false;
    }
  }
});
